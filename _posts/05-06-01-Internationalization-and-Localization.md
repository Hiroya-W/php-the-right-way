...
- [php-gettext/Gettext][php-gettext]: OOインタフェースでのGettextサポート。改良されたヘルパー関数、いくつかのファイルフォーマットのための強力な抽出器（`gettext` コマンドによるネイティブにサポートしないものもある）を含み、`.mo/.po` ファイル以外の他のフォーマットにエクスポートすることもできる。翻訳ファイルをJavaScriptインターフェースなどのシステムの他の部分に統合する必要がある場合に役に立つ。
